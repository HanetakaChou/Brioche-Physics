...
public:
	...
	inline void physics_world_step(float delta_time);
};
...
public:
	...
	inline void apply_key_frame(float const rotation[4], float const position[3]);
...
{
	btAlignedAllocSetCustom(_internal_alloc, _internal_free);
...
extern "C" void brx_physics_world_step(brx_physics_context *, brx_physics_world *physics_world, float delta_time)
{
	physics_world->physics_world_step(delta_time);
}
...
{
	physics_rigid_body->apply_key_frame(rotation, position);
}
...
inline void brx_physics_world::physics_world_step(float delta_time)
{
	...
	this->m_dynamics_world->stepSimulation(delta_time, max_sub_steps, fixed_time_step);
}
...
inline void brx_physics_rigid_body::init(brx_physics_world *physics_world, float const wrapped_rotation[4], float const wrapped_position[3], BRX_PHYSICS_RIGID_BODY_SHAPE_TYPE wrapped_shape_type, float const wrapped_shape_size[3], BRX_PHYSICS_RIGID_BODY_MOTION_TYPE wrapped_motion_type, uint32_t wrapped_collision_filter_group, uint32_t wrapped_collision_filter_mask, float wrapped_mass, float wrapped_linear_damping, float wrapped_angular_damping, float wrapped_friction, float wrapped_restitution)
{
	...
	btTransform const transform(btQuaternion(wrapped_rotation[0], wrapped_rotation[1], wrapped_rotation[2], wrapped_rotation[3]), btVector3(wrapped_position[0], wrapped_position[1], wrapped_position[2]));
...
inline void brx_physics_rigid_body::apply_key_frame(float const wrapped_rotation[4], float const wrapped_position[3])
{
	btTransform const world_transform(btQuaternion(wrapped_rotation[0], wrapped_rotation[1], wrapped_rotation[2], wrapped_rotation[3]), btVector3(wrapped_position[0], wrapped_position[1], wrapped_position[2]));
	this->m_rigid_body->getMotionState()->setWorldTransform(world_transform);
}

inline void brx_physics_rigid_body::get_body_transform(float out_rotation[4], float out_position[3]) const
{
	btTransform world_transform;
	this->m_rigid_body->getMotionState()->getWorldTransform(world_transform);

	btQuaternion world_rotation = world_transform.getRotation();
	out_rotation[0] = world_rotation.getX();
	out_rotation[1] = world_rotation.getY();
	out_rotation[2] = world_rotation.getZ();
	out_rotation[3] = world_rotation.getW();

	btVector3 world_translation = world_transform.getOrigin();
	out_position[0] = world_translation.getX();
	out_position[1] = world_translation.getY();
	out_position[2] = world_translation.getZ();
}
...
inline void brx_physics_constraint::init(brx_physics_world *physics_world, btRigidBody *rigid_body_a, btRigidBody *rigid_body_b, BRX_PHYSICS_CONSTRAINT_TYPE wrapped_constraint_type, float const wrapped_pivot[3], float const wrapped_twist_axis[3], float const wrapped_plane_axis[3], float const wrapped_normal_axis[3], float const wrapped_twist_limit[2], float const wrapped_plane_limit[2], float const wrapped_normal_limit[2])
{
	...
	btAssert(NULL == this->m_constraint);
	{
		...
		case BRX_PHYSICS_CONSTRAINT_FIXED:
		{
			...
			btAssert(std::abs(wrapped_twist_limit[0]) < epsilon);
			btAssert(std::abs(wrapped_twist_limit[1]) < epsilon);
			btAssert(std::abs(wrapped_plane_limit[0]) < epsilon);
			btAssert(std::abs(wrapped_plane_limit[1]) < epsilon);
			btAssert(std::abs(wrapped_normal_limit[0]) < epsilon);
			btAssert(std::abs(wrapped_normal_limit[1]) < epsilon);
